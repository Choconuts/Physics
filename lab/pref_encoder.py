...
class PREF(nn.Module):
    def __init__(self, linear_freqs, reduced_freqs=[1] * 3, feature_dim=16, sampling='linear', device='cuda',
                 **kwargs) -> None:
        """
        ...
        if sampling == 'linear':
            self.axis = [torch.tensor([0.] + [i + 1 for i in torch.arange(d - 1)]).to(self.device) for d in
                         reduced_freqs]
        else:
            self.axis = [torch.tensor([0.] + [2 ** i for i in torch.arange(d - 1)]).to(self.device) for d in
                         reduced_freqs]
    ...
    def compute_fft(self, features, xyz_sampled, interp=True):
        if interp:
            ...
            volume = fx + fy + fz
            points = F.grid_sample(volume, xyz_sampled[None, None, None].flip(-1), align_corners=True).view(-1,
                                                                                                            *xyz_sampled.shape[
                                                                                                             :1], )
            ...
            fx = grid_sample_cmplx(Fx.transpose(3, 3).flatten(1, 2), torch.stack([zs, ys], dim=-1)[None]).reshape(
                Fx.shape[1], Fx.shape[2], -1)
            fy = grid_sample_cmplx(Fy.transpose(2, 3).flatten(1, 2), torch.stack([zs, xs], dim=-1)[None]).reshape(
                Fy.shape[1], Fy.shape[3], -1)
            fz = grid_sample_cmplx(Fz.transpose(2, 4).flatten(1, 2), torch.stack([xs, ys], dim=-1)[None]).reshape(
                Fz.shape[1], Fz.shape[4], -1)
